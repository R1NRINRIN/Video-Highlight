...
class D2GCN(nn.Module):
    ...
    def build_graph(self,num_nodes,device):
        ...
        # /*
        self.g.update_all(self.send_source, self.simple_reduce)   # message_func, reduce_func, apply_node_func
        # */
        # self.g.register_reduce_func(self.simple_reduce)

    def send_source(self, edges):
        print("edges ======", edges.dst)
        edge_feature = self.fedge.forward(torch.cat((edges.src["h"], edges.dst["h"]),dim=1))   # 행 방향
